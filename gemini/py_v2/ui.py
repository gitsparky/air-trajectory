...
def calculate_prediction():
    ...
    theta = float(theta_pred_entry.get())
    h0 = float(h0_pred_entry.get())

    R, H, X_max = pc.calculate_prediction(v0, theta, h0)

    prediction_results_label.config(text=f"Range: {R:.2f} m\nMax Height: {H:.2f} m\nDistance at Max Height: {X_max:.2f} m")
    draw_trajectory(v0, np.radians(theta), h0)

def calculate_calibration():
    R = float(range_cal_entry.get())
    theta = float(theta_cal_entry.get())
    h0 = float(h0_cal_entry.get())

    v0 = pc.calculate_calibration(R, theta, h0)
...
def calculate_targeting():
    ...
    v0, theta = pc.calculate_targeting(x, y, h0)
...
def draw_trajectory(v0, theta, h0):
    ...
    y_values = h0 + v0 * np.sin(theta) * t - 0.5 * pc.g * t**2
    ...
    T_max = v0 * np.sin(theta) / pc.g
    H = h0 + (v0**2 * np.sin(theta)**2) / (2 * pc.g)
    X_max = (v0**2 * np.sin(2 * theta)) / (2 * pc.g)
    T_range = (v0 * np.sin(theta) + np.sqrt(v0**2 * np.sin(theta)**2 + 2 * pc.g * h0)) / pc.g
...
exit_button = tk.Button(root, text="Exit", command=lambda x: exit())
